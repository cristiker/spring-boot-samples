 ...
 */
package com.cristik.common.http;

import com.cristik.common.collect.MapUtil;
import com.cristik.common.io.PropertiesUtil;
import com.cristik.common.lang.StringUtil;
import com.cristik.common.mapper.JsonMapper;
import com.cristik.common.mapper.XmlMapper;
...
//			response.reset(); // 先注释掉，否则以前设置的Header会被清理掉，如ajax登录设置记住我Cookie
	...
	public static void setExpiresHeader(HttpServletResponse response, long expiresSeconds) {
		...
		response.setDateHeader(HttpHeader.EXPIRES, System.currentTimeMillis() + expiresSeconds * 1000);
		// Http 1.1 header, set a time after now.
		response.setHeader(HttpHeader.CACHE_CONTROL, "private, max-age=" + expiresSeconds);
	}
	...
	public static void setNoCacheHeader(HttpServletResponse response) {
		// Http 1.0 header
		response.setDateHeader(HttpHeader.EXPIRES, 1L);
		response.addHeader(HttpHeader.PRAGMA, "no-cache");
		// Http 1.1 header
		response.setHeader(HttpHeader.CACHE_CONTROL, "no-cache, no-store, max-age=0");
	}
	...
	public static void setLastModifiedHeader(HttpServletResponse response, long lastModifiedDate) {
		response.setDateHeader(HttpHeader.LAST_MODIFIED, lastModifiedDate);
	}
	...
	public static boolean checkIfModifiedSince(HttpServletRequest request, HttpServletResponse response,
			long lastModified) {
		long ifModifiedSince = request.getDateHeader(HttpHeader.IF_MODIFIED_SINCE);
	...
	public static boolean checkIfNoneMatchEtag(HttpServletRequest request, HttpServletResponse response, String etag) {
		String headerValue = request.getHeader(HttpHeader.IF_NONE_MATCH);
		if (headerValue != null) {
